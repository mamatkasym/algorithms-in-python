...
def total_strength(arr: list[int]) -> int:
    """

    :param arr:
    :return: total sum of all subarrays' strength, product of its minimum element and sum of all elements, or
             sum(sum(arr[i...j]) * min(arr[i...j]) for all 0 <= i <= j < len(arr))
    """
